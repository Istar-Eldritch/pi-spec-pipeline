...
describe("formatBox", () => {
	...
	it("wraps long lines", () => {
		const longLine =
			"This is a very long line that should be wrapped to fit within the box width";
...
describe("formatModelConfig", () => {
	it("formats model with thinking level", () => {
		expect(formatModelConfig({ model: "gpt-5.5", thinking: "high" })).toBe(
			"gpt-5.5/high",
		);
	});

	it("formats gpt-5.4 with medium thinking", () => {
		expect(formatModelConfig({ model: "gpt-5.4", thinking: "medium" })).toBe(
			"gpt-5.4/medium",
		);
	});

	it("formats gpt-5.4-mini with off thinking", () => {
		expect(formatModelConfig({ model: "gpt-5.4-mini", thinking: "off" })).toBe(
			"gpt-5.4-mini/off",
		);
	});
});
