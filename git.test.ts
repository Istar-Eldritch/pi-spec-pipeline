...
import { extractCommitMessage, captureGitStatus, getModifiedFiles, stageFiles, hasChangesStaged, stashChanges, dropStash, stashExists, resetToHead, execGit } from "./git.ts";
...
describe("Git file tracking utilities", () => {
	...
	describe("Error recovery operations", () => {
		...
		it("resetToHead handles modified tracked files", async () => {
			...
			await writeFile(join(testDir, "README.md"), "# Completely different content\n");
	...
	describe("integration: file tracking workflow", () => {
		it("tracks files modified by agent simulation", async () => {
			...
			await writeFile(join(testDir, "new-feature.ts"), "export const feature = true;\n");
			...
			const commitResult = await execGit(testDir, ["commit", "-m", "Agent changes"]);
